...
class CompetitionView(viewsets.ModelViewSet):
    ...
    def user_comps(self, request):
        competitions=Competition.objects.filter(scores__user_id= request\
            .data.get('user_id')).order_by('-scores__last_updated')

        page = self.paginate_queryset(competitions)
        if page is not None:
            serializer = UserCompsSerializer(page, many=True,\
                 context={"user_id": request.data.get('user_id')})
            return self.get_paginated_response(serializer.data)

        serializer = UserCompsSerializer(competitions, many=True,\
             context={"user_id": request.data.get('user_id')})
